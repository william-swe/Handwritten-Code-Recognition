...
def collect_levenshtein_distances():
    ...
    services = [OcrService.AZURE, OcrService.MISTRAL, OcrService.OPENAI, OcrService.ANTHROPIC]
    ...
    # Calculate averages
    avg_row = {service: (sum(avg_ld[service])/len(avg_ld[service]) if avg_ld[service] else '') for service in services}
...
def get_average_normalized_levenshtein(service_name: str, output_lines, summary_gt_files=None):
    ...
    if table_rows:
        ...
        output_lines.append(f"| **Average** |  |  | **{avg_nld:.4f}** |\n")
        output_lines.append("\n")
...
if __name__ == "__main__":
    ...
    output_lines.append("| Ground Truth File | Azure LD | Mistral LD | GPT LD | Claude LD |\n")
    output_lines.append("|:---:|:---:|:---:|:---:|:---:|\n")
    summary_gt_files = []
    ...
    avg_row_fmt = [f"{avg_row[service]:.2f}" if avg_row[service] != '' else "-" for service in services]
